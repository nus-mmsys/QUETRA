...
minIa <- matrix(c(0,0,0,0,0,0,0,0,0), ncol=1)
maxIa <- matrix(c(0,0,0,0,0,0,0,0,0), ncol=1)
Ib <- matrix(c(" "," "," "," ", " "," "," "," "," "), ncol=1)
...
minIIa <- matrix(c(0,0,0,0,0,0,0,0,0), ncol=1)
maxIIa <- matrix(c(0,0,0,0,0,0,0,0,0), ncol=1)
...
minIIIa <- matrix(c(0,0,0,0,0,0,0,0,0), ncol=1)
maxIIIa <- matrix(c(0,0,0,0,0,0,0,0,0), ncol=1)
...
for (i in 1:9)
{
  ...
  HIdf[i,j+1]<-Iv[i+j*9,]
  HIIdf[i,j+1]<-IIv[i+j*9,]
  HIIIdf[i,j+1]<-IIIv[i+j*9,]
...
for(k in 1:9)
{
  ...
  Ia[k]<-sum(HIdf[k,(7*j)+1],HIdf[k,(7*j)+2],HIdf[k,(7*j)+3],HIdf[k,(7*j)+4],HIdf[k,(7*j)+5],HIdf[k,(7*j)+6],HIdf[k,(7*j)+7])/7
...
Iaa<-barplot(Ia, width = 5, space =0.5, beside=TRUE , main=paste("Profile:",j+1," Metric: # Quality Change", "\n","Buffer: 30/60 "), density = 100 ,ylim=c(0,300), ylab="Number of Change", font.lab = 2, axisnames=Ia,col=brewer.pal(9, "Set1")) 
arrows(Iaa, minIa, Iaa,
       maxIa, lwd = 1.5, angle = 90,
       code = 3, length = 0.05)

#par(mar = c(4, 1, 4, 1))
IIaa<-barplot(IIa, width = 5, space =0.5, beside=TRUE , main=paste("Profile:",j+1," Metric:  # Quality Change", "\n","Buffer: 120"), density = 100 ,ylim=c(0,300), ylab="Number of Change", font.lab = 2, axisnames=IIa,col=brewer.pal(9, "Set1")) 
arrows(IIaa, minIIa, IIaa,
       maxIIa, lwd = 1.5, angle = 90,
       code = 3, length = 0.05)

#par(mar = c(4, 1, 4, 1) + 0.4)
IIIaa<-barplot(IIIa, width = 5, space =0.5, beside=TRUE , main=paste("Profile:",j+1," Metric:  # Quality Change", "\n","Buffer: 240 "), density = 100 ,ylim=c(0,300),ylab="Number of Change", font.lab = 2, axisnames=IIIa,col=brewer.pal(9, "Set1")) 
arrows(IIIaa, minIIIa, IIIaa,
       maxIIIa, lwd = 1.5, angle = 90,
       code = 3, length = 0.05)
...
IIIIaa<-barplot(dummy, axes=FALSE,col=brewer.pal(9, "Set1"),legend.text = Ib,
