...
for (i in 1:9)
{
  ...
  HIdf[i,j+1]<-Ix[i+j*9,]
  HIIdf[i,j+1]<-IIx[i+j*9,]
  HIIIdf[i,j+1]<-IIIx[i+j*9,]
...
Iaa<-barplot(Ia, width = 5, space =0.5, beside=TRUE , main=paste("Profile:",j+1," Metric: Avg-Bitrate", "\n","Buffer: 30/60 "), density = 100 ,ylim=c(0,4400), ylab="Kbps", font.lab = 2,axisnames=Ia,col=brewer.pal(9, "Set1")) 
arrows(Iaa, minIa, Iaa,
       maxIa, lwd = 1.5, angle = 90,
       code = 3, length = 0.05)

#par(mar = c(4, 1, 4, 1))
IIaa<-barplot(IIa, width = 5, space =0.5, beside=TRUE , main=paste("Profile:",j+1," Metric: Avg-Bitrate", "\n","Buffer: 120"), density = 100 ,ylim=c(0,4400), ylab="Kbps",font.lab = 2, axisnames=IIa,col=brewer.pal(9, "Set1")) 
arrows(IIaa, minIIa, IIaa,
       maxIIa, lwd = 1.5, angle = 90,
       code = 3, length = 0.05)

#par(mar = c(4, 1, 4, 1) + 0.4)
IIIaa<-barplot(IIIa, width = 5, space =0.5, beside=TRUE , main=paste("Profile:",j+1," Metric: Avg-Bitrate", "\n","Buffer: 240 "), density = 100 ,ylim=c(0,4400), ylab="Kbps",font.lab = 2, axisnames=IIIa,col=brewer.pal(9, "Set1")) 
