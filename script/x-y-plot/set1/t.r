...
Cy<-union(Cy,IIIe)
 ...
 Ip=xyplot(Ie~Ia, main = paste("\n         Buffer: 30/60"), data=data.frame(Ie,Ia), pch=19, xlab=" ", ylab=yl,  font.lab=2,group=IIIb, cex=(IinEFF),  scales=list(cex=c(0.7,0.7)),ylim=c(min(Cy), max(Cy)),xlim=c(min(Cx), max(Cx)) )

 IIp=xyplot(IIe~IIa, main = paste("    Profile:",toupper(substr(IIz[i+j*9,],2,2)),",  Sample:",toupper(substr(IIu[i+j*9,],2,2)),"\n","    Buffer: 120"), data=data.frame(IIe,IIa), pch=19, xlab=xl, ylab=NULL, font.lab=2,group=IIIb, cex=(IIinEFF),  scales=list(cex=c(0.7,0.7)),ylim=c(min(Cy), max(Cy)),xlim=c(min(Cx), max(Cx)))
...
my.settings <- list(
 ...
 IIIp=xyplot(IIIe~IIIa, par.settings=my.settings,main = paste("\n                       Buffer: 240"),  data=data.frame(IIIe,IIIa), pch=19,key = simpleKey(sort(IIIb),space="right",cex=c(0.7,0.7)), xlab=" ", ylab=NULL,group=IIIb, cex=(IIIinEFF),  scales=list(cex=c(0.7,0.7)),ylim=c(min(Cy), max(Cy)),xlim=c(min(Cx), max(Cx)))
