...
postscript(file=paste(toupper(IIz[i+j*9,]),toupper(IIu[i+j*9,]),".eps"),onefile=FALSE, horizontal=FALSE,paper="special",width=12,height=4);
...
Cy<-union(Cy,IIIe)
 ...
 Ip=xyplot(Ie~Ia, main = paste("\nBuffer: 30/60"), data=data.frame(Ie,Ia), pch=20, xlab=" ", ylab=yl,  font.lab=2,group=Ib, cex=(IinEFF),  scales=list(cex=c(0.7,0.7)),ylim=c(min(Cy), max(Cy)),xlim=c(min(Cx), max(Cx)) )

 IIp=xyplot(IIe~IIa, main = paste("Profile:",toupper(substr(IIz[i+j*9,],2,2)),",  Sample:",toupper(substr(IIu[i+j*9,],2,2)),"\n","Buffer: 120"), data=data.frame(IIe,IIa), pch=20, xlab=xl, ylab=NULL, font.lab=2,group=IIb, cex=(IIinEFF),  scales=list(cex=c(0.7,0.7)),ylim=c(min(Cy), max(Cy)),xlim=c(min(Cx), max(Cx)))


 IIIp=xyplot(IIIe~IIIa, main = paste("\nBuffer: 240"), data=data.frame(IIIe,IIIa), pch=20,key = simpleKey(toupper(IIIb),space="right",cex=c(0.7,0.7)), xlab=" ", ylab=NULL,group=IIIb, cex=(IIIinEFF),  scales=list(cex=c(0.7,0.7)),ylim=c(min(Cy), max(Cy)),xlim=c(min(Cx), max(Cx)))
...
#It,
grid.arrange(Ip,IIp,IIIp, ncol=3,widths=c(2,2,2.5))
#dev.off()

dev.off.crop(file=paste(toupper(IIz[i+j*9,]),toupper(IIu[i+j*9,]),".eps"))
}
