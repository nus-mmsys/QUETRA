...
plt <- plt + geom_point(data=dt,
                       aes(x=metricx,
                           y=metricy,
                           color=Algorithms, shape = Algorithms),size =12, stroke = 3)+ xlim(min(dt$metricx)-(0.1*min(dt$metricx)), max(dt$metricx)+(0.1*max(dt$metricx))) + scale_shape_manual(values = c(0, 6, 1, 5,2))
