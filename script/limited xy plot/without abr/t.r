...
Ix<-Idf[4];
Iineff<-Idf[5];
Ia <- matrix(c(0,0,0,0,0), ncol=1)
Ib <- matrix(c(" "," "," "," ", " "), ncol=1)
Ie <- matrix(c(0,0,0,0,0), ncol=1)
IinEFF <- matrix(c(0,0,0,0,0), ncol=1)
...
IIx<-IIdf[4];
IIineff<-IIdf[5];
IIa <- matrix(c(0,0,0,0,0), ncol=1)
IIb <- matrix(c(" "," "," "," ", " "), ncol=1)
IIe <- matrix(c(0,0,0,0,0), ncol=1)
IIinEFF <- matrix(c(0,0,0,0,0), ncol=1)
...
IIIx<-IIIdf[4];
IIIineff<-IIIdf[5];
IIIa <- matrix(c(0,0,0,0,0), ncol=1)
IIIb <- matrix(c(" "," "," "," ", " "), ncol=1)
IIIe <- matrix(c(0,0,0,0,0), ncol=1)
IIIinEFF <- matrix(c(0,0,0,0,0), ncol=1)
...
for (i in 1:5)
{
 ...
 #IIb[i]<-paste(IIy[i+j*9,])
  IIe[i]<-IIv[i+j*5,]
  IIinEFF[i]<-(7*IIineff[i+j*5,]) 


  IIIa[i]<-IIIx[i+j*5,]
 #IIIb[i]<-paste(IIIy[i+j*9,])
  IIIe[i]<-IIIv[i+j*5,]
  IIIinEFF[i]<-(7*IIIineff[i+j*5,]) 
	  ...
	  IIb[1]<-paste("QUETRA-Avg Throughput")
	
	  IIb[2]<-paste("Buffer Based")
	
	  IIb[3]<-paste("BOLA-Last")
	
	
	  IIb[4]<-paste("ELASTIC-Last")
	
	  IIb[5]<-paste("QUETRA-Last")
	  ...
	  IIIb[1]<-paste("QUETRA-Avg Throughput")
	
	  IIIb[2]<-paste("Buffer Based")
	
	  IIIb[3]<-paste("BOLA-Last")
	
	
	  IIIb[4]<-paste("ELASTIC-Last")
	
	  IIIb[5]<-paste("QUETRA-Last")
