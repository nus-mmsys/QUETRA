...
postscript(file=str_replace_all(paste("new",".eps"), fixed(" "), ""),onefile=FALSE, horizontal=FALSE,paper="special",width=12,height=4);
...
Cy<-union(Cy,IIIe)
 ...
 Ip=xyplot(Idf$change~Idf$avgR, main = paste("\n         Buffer: 30/60"), data=data.frame(Idf$change,Idf$avgR), pch=1, xlab=" ", ylab=yl,  font.lab=2,group=Idf$method, cex=(Idf$ineff),  scales=list(cex=c(0.7,0.7)),ylim=c(-20, max(Idf$change)+37),xlim=c(min(Idf$avgR)-250, max(Idf$avgR)+260) )

head(Idf)
head(IIdf)
head(IIIdf)

Ix=xyplot(Idf$change~Idf$avgR)

 #IIp=xyplot(IIe~IIa, main = paste("       Overall ","\n","    Buffer: 120"), data=data.frame(IIe,IIa), pch=1, xlab=xl, ylab=NULL, font.lab=2,group=Ib, cex=(IIinEFF),  scales=list(cex=c(0.7,0.7)),ylim=c(0, max(Cy)+37),xlim=c(0, max(Cx)+260))
...
#my.settings <- list(
 ...
 #IIIp=xyplot(IIIe~IIIa, par.settings=my.settings,main = paste("\n                       Buffer: 240"),  data=data.frame(IIIe,IIIa), pch=1,key = simpleKey(sort(Ib),space="right",cex=c(0.75,0.75)), xlab=" ", ylab=NULL,group=Ib, cex=(IIIinEFF),  scales=list(cex=c(0.7,0.7)),ylim=c(0, max(Cy)+37),xlim=c(0, max(Cx)+260))
