...
filename <- paste("legend", '-avgm.pdf', sep="")
...
plt <- plt + geom_point(data=dt,
                       aes(x=metricx,
                           y=metricy,
                           color=method),size =5, shape=43, stroke = 2)+ xlim(min(dt$metricx)-100, max(dt$metricx)+100) 
...
plt <- plt    + theme_bw() + 
  ...
  theme(axis.line.x = element_line(color="black", size = 0.5),
        axis.line.y = element_line(color="black", size = 0.5)) +theme(legend.margin=unit(c(-14,1,0,5),"mm")) +theme(plot.margin=unit(c(1,16,0.2,-4),"cm"))+theme(  legend.key = element_rect(fill = "white", colour = "white"))+guides(colour = guide_legend(override.aes = list(size = 5) ))  +theme(axis.text=element_text(size=12,face="bold"), axis.title=element_text(size=18,face="bold"))

plt <- plt +  theme(legend.key.size = unit(2, "cm")) + theme(legend.key.height= unit(2, "cm")) + theme(legend.key.width= unit(2, "cm"))

#plt <- plt + theme(plot.margin = unit(c(1,1,1,1), "cm"))
plt <- plt + scale_size(range = c(4,4))
plt <- plt +  scale_color_manual(labels = Ib,values = dt$method,name="" )

plt <- plt + theme(legend.text=element_text(size=18))
