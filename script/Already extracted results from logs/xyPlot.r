...
plt <- plt + geom_point(data=dt,
                       aes(x=metricx,
                           y=metricy,
                           color=method),size =dt$metricSize, shape=1, stroke = 2)+ xlim(min(dt$metricx)-100, max(dt$metricx)+50) 

plt <- plt + geom_point(data=dt,
                       aes(x=metricx,
                           y=metricy,
                           color=method),size =5, shape=43, stroke = 2)+ xlim(min(dt$metricx)-100, max(dt$metricx)+50) 
