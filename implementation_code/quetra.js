...
function Quetra(config) {
    ...
    let instance,
        ...
        throughputTimeArray,////Array to store the arrrival time of segment
        bitrateArray, //Array to store bitrate of downloaded segment
        av, // last value of throughput
        bv, // last value of bitrate
        alpha, //weigth factor for EMA
        alphaMax,//maximum value of alpha during playback
        adapter; //Default
    var slack = []; //Array for storing Buffer slack
    var slack30 = []; //Array for storing Buffer slack for capacity of 30sec
    var slack60 = []; //Array for storing Buffer slack fir capacity of 60 sec
    var slack120 = []; //Array for storing Buffer slack for capacity of 120sec
    var slack240 = [];  //Array for storing Buffer slack for capacity of 240sec
    ...
    function setup() {
        ...
        bv = 0;
        alpha = 0.1;
        alphaMax = 0.1;//intial value for maximum value of alpha
    ...
    function averageThroughputByTypeLastThroughput(type) {
    ...
    function averageThroughputByTypeAverageOfLastThree(type) {
        var arrThroughput = throughputArray[type];
        var lenThroughput = arrThroughput.length;
        if (lenThroughput < 2) {
            av = arrThroughput[lenThroughput - 1];
    ...
    //EMA
    function averageThroughputByTypeEMA(type) {
        var arrThroughput = throughputArray[type];
        var lenThroughput = arrThroughput.length;
        if (lenThroughput < 2) {
            av = arrThroughput[lenThroughput - 1];
    ...
    //Gradient Based EMA
    function averageThroughputByTypeGradientBasedEMA(type) {
        var arrThroughput = throughputArray[type];
        ...
        if (lenThroughput < 2) {
            ...
            av = (av * (1 - alpha)) + (alpha * arrThroughput[lenThroughput - 1]);
        }
        return (av);
    }

    //Low Pass EMA
    function averageThroughputByTypeLowPassEMA(type) {
        var arrThroughput = throughputArray[type];
        var arrTimeThroughput = throughputTimeArray[type];
        var lenThroughput = arrThroughput.length;
        var mNorm,mInst;
    ...
    //KAMA
    function averageThroughputByTypeKAMA(type) {
        var arrThroughput = throughputArray[type];
        var lenThroughput = arrThroughput.length;
        var window,er,fsc,ssc,sc,change,valatality;
        if (lenThroughput < 2) {
            av = arrThroughput[lenThroughput - 1];
        }
        else if (lenThroughput == 2) {
    ...
    function execute (rulesContext, callback) {
        ...
        var rho;
        var rhof = 0; //selected bitrates's rho variables
        let i;//trmp variable
        var minDiff,minIndex; //temperoray variables
        var lowResRatio = 90 / 240;//low reservoir ratio from BBA
        ...
        av = averageThroughputByTypeLastThroughput(mediaType) / 1000;  //change fuction to get different throughput prediction and smoothing
        ...
        if ( buff < lowResRatio * bufferMax) //Same lower resrvior maintainence as  BBA
        {
            minIndex = 0;
        }
        ...
        if (abrController.getAbandonmentStateFor(mediaType) !== AbrController.ABANDON_LOAD) {
            if (bufferStateVO.state === BufferController.BUFFER_LOADED || isDynamic) {
                if (count % 5 == 0 || buff < (90 / 240) * bufferMax) {//decision at 5th segments arrival or if lower reservior is needed to be maintained
